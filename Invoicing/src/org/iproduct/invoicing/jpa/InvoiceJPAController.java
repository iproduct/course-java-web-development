...
public class InvoiceJPAController{
	...
	public Invoice create(Invoice invoice) throws EntityAlreadyExistsException {
		EntityManager em = JPAMainController.getEntityManager();
		EntityTransaction transaction = em.getTransaction();
		transaction.begin();
		try{
			//check if invoice entity is new one
			if(em.find(Invoice.class, invoice.getNumber()) != null)
				throw new EntityAlreadyExistsException("Invoice number = " 
						+ invoice.getNumber() + " already exists.");
			
			//persist invoice
			em.persist(invoice);

			//fix proper connections with positions referred by this invoice
			List<Position> positions = invoice.getPositions();
			if(positions != null){
				int pNumber = 0;
				for(Position p : positions){
					p.setInvoice(invoice);
					p.setId(new PositionPK(++pNumber, invoice.getNumber()));
					JPAMainController.getPositionController().create(p);
				}
			}
