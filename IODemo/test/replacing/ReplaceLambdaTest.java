...
public class ReplaceLambdaTest {
	...
	public final void testReplaceOccurencesFromInfileToOutfile() throws IOException {
		...
		//3. Postcondition
		assertTrue(
				"Result file does exist: " + RESULT_FILE_NAME,
				Files.exists(Paths.get(RESULT_FILE_NAME)) 
			);
		assertFalse(
				"Result file does not contain search expression: " + SEARCH_EXPRESSION,
				containsExpression(RESULT_FILE_NAME, SEARCH_EXPRESSION)
			);
		assertTrue(
				"Result file does contain replacement expression: " + REPLACEMENT_EXPRESSION,
