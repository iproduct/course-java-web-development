...
public interface UserService {
	User[] getAllUsers(); // returns all created Users in the system;
	User getUserById(long id); // returns the User with selected id if such exists, or throws NonexistingEntityException otherwise, with appropriate message;
	User getUserByEmail(String email); // returns the User with selected email if such exists, or throws NonexistingEntityException otherwise, with appropriate message;
	User createUser(User user); // adds new user provided as argument, and returns the User with auto-generated id filled in, can throw InvalidEntityDataException when some of the required User fields are missing, or when the email or password are invalid (choose your own validation patterns), or when some of the resource ids referred by some Permission are not valid Resource ids;
	User updateUser(User user); // updates new user with provided data (as argument), and returns the updated User, or throws NonexistingEntityException with appropriate message if User with given id does not exist. Can throw InvalidEntityDataException when some of the reqired User fields are missing, or when the password is invalid, or when some of the resource ids referred by some Permission are not valid Resource ids. The email property should be immutable; // should not be allowed to be changed; // in case such attempt is made PropertyChangeNotAllowedException should be thrown with appropriate message;
	User deleteUserById(long id); // deletes the User with selected id and returns it, if such User exists, or throws NonexistingEntityException otherwise, with appropriate message;
